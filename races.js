...
async function loadRaceDetails(raceId, raceName, raceDate) {
  ...
  // Render the tables
  renderQualifyingTable(raceId, qualifying);
  renderResultsTable(raceId, results, podiumFixed);
}
...
function renderQualifyingTable(raceId, qualifying) {
  const qualifyingContainer = document.getElementById("qualifyingContainer");
    ...
    <table>
      <thead>
        <tr>
          ...
          <th onclick="sortTable('qualifying_${raceId}', 'circuitName')">Show Circuit</th>
      ...
      <tbody>
        ${qualifying
          .map((q) => {
            ...
            const isFavCircuit = isFavorite('circuit', q.race.id);
            ...
            return `
              <tr>
                ...
                <td>
                  <span onclick="toggleFavorite('circuit', ${q.race.id})" style="cursor:pointer;">${circuitStar}</span>
                  <a onclick="showCircuitModalFromAPI('${q.race.id}')">Show Circuit</a>
                </td>
                <td>${q.q1}</td>
                <td>${q.q2}</td>
                <td>${q.q3}</td>
...
function renderResultsTable(raceId, results, fixedPodium) {
    ...
    <table>
      <thead>
        <tr>
          ...
          <th onclick="sortTable('results_${raceId}', 'circuitName')">Show Circuit</th>
      ...
      <tbody>
        ${results
          .map((r) => {
            ...
            const isFavCircuit = isFavorite('circuit', r.race.id);
            const circuitStar = isFavCircuit ? '★' : '☆';
            ...
            return `
              <tr>
                ...
                <td>
                  <span onclick="toggleFavorite('circuit', ${r.race.id})" style="cursor:pointer;">${circuitStar}</span>
                  <a onclick="showCircuitModalFromAPI('${r.race.id}')">Show Circuit</a>
                </td>
                <td>${r.laps}</td>
                <td>${r.points}</td>
              </tr>
...
function sortTable(storageKey, columnKey) {
  ...
  if (storageKey.startsWith("qualifying")) {
    renderQualifyingTable(storageKey.split("_")[1], sortedData);
  } else if (storageKey.startsWith("results")) {
    renderResultsTable(storageKey.split("_")[1], sortedData, fixedPodium);
  }
...
function showDriverModal(driverId, driverRef, seasonYear, points) {
  ...
  fetch(driverDetailsUrl)
    ...
    .then((driver) => {
      ...
      const isFavDriver = isFavorite('driver', driverId);
      const favoriteStar = document.createElement('span');
      favoriteStar.innerHTML = isFavDriver ? '★' : '☆';
      favoriteStar.style.cursor = 'pointer';
      favoriteStar.style.marginLeft = '10px';
      favoriteStar.onclick = () => {
        toggleFavorite('driver', driverId);
        favoriteStar.innerHTML = isFavorite('driver', driverId) ? '★' : '☆';
      };
      document.getElementById('driverName').appendChild(favoriteStar);
      ...
      // Validate and render the URL
      if (driver.url) {
        driverURLAnchor.href = driver.url;
        driverURLAnchor.innerText = "Wikipedia";
        driverURLAnchor.target = "_blank"; // Open in a new tab
      } else {
        driverURLAnchor.href = "#";
  ...
  fetch(url)
    .then((response) => {
      if (!response.ok) throw new Error(`Failed to fetch driver results: ${response.status}`);
      return response.json();
    })
    .then((data) => {
      if (data.length > 0) {
          ...
          .map(
            (result) => `
              <tr>
                ...
                <td>${result.points !== undefined ? result.points : points || 'N/A'}</td>
              </tr>`
          )
          .join('');
